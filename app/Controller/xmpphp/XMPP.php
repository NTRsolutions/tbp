	...
	protected function vcard_get_handler($xml) {
	 ...
	 public function registerNewUser($username, $password = NULL){
        ...
        */
		...
		public function addRosterContact($jid, $name, $groups = array()) {
			...
			// return if there is no jid specified
               if (!$jid) { return; }
                ...
                $xml = "<iq type='set' id='$id'>";
                $xml .= "<query xmlns='jabber:iq:roster' subscription='both'>";
                $xml .= "<item jid='$jid' name='$name' subscription='both' approved='true'>";
				$xml .= "<group>$group</group>";
				/*$xml .= "<group>Friends</group>";
		...
		public function addRosterContactSendFriendRequest($jid,$name,$groups = array()){
				if (!$name) { 
					$name = $jid;
				}
				
                $id = $this->getID();
				$xml="<iq  type='set'><query xmlns='jabber:iq:roster'><item jid='$jid' name='$name'><group>Friends</group></item></query></iq>
				<presence to='$jid' type='subscribe'></presence>";
				$this->addIdHandler($id, 'add_roster_contact_handler');
				$this->send($xml);
		}
		public function addRosterContactAccpectFriendRequest($jid,$name,$groups = array()){
				if (!$name) { 
					$name = $jid;
				}
                $id = $this->getID();
				$xml="<iq type='set'><query xmlns='jabber:iq:roster'><item jid='$jid' name='$name'><group>Friends</group></item></query></iq>
				<presence  to='$jid' type='subscribe'></presence>
				<presence to='$jid' type='subscribed'></presence>";
				
				$this->addIdHandler($id, 'add_roster_contact_handler');
				$this->send($xml);
		}
		
		public function removeFromContact($from_email,$to_email){
				$id = $this->getID();
				$xml="<iq from='$from_email' id='$id' type='set'>
					 <query xmlns='jabber:iq:roster'>
					   <item jid='$to_email' subscription='remove'/>
					 </query>
				   </iq>";
		...
		public function addRosterContactNew($jid, $name, $groups = array()) {
			 
			 $id="hm4hs97y";
			 $xml='<iq from="as@ejabberd.rapidsoft.in"
					   id="$id"
					   type="set">
					 <query xmlns="jabber:iq:roster">
					   <item jid="ask@ejabberd.rapidsoft.in"
							 subscription="remove"/>
					 </query>
				   </iq>';
			/*$xml='<iq type="set"><query xmlns="jabber:iq:roster"><item jid="ask@ejabberd.rapidsoft.in" name="ask"><group>Friends</group></item></query></iq>
<presence  to="ask@ejabberd.rapidsoft.in" type="subscribe"></presence>
<presence to="ask@ejabberd.rapidsoft.in" type="subscribed"></presence>';
				*/
			$this->addIdHandler($id, 'add_roster_contact_handler');
			$this->send($xml);
			die();
